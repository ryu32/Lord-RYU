...
async def deploy(event, repo, ups_rem, ac_br, txt):
    if HEROKU_API_KEY is not None:
        ...
        if build.status == "failed":
            ...
            await event.edit("`Lord-RYU Berhasil Di Deploy!\n" "Restarting, Mohon Menunggu Kawan.....`")
...
async def update(event, repo, ups_rem, ac_br):
    ...
    await asyncio.sleep(1)
    await event.edit('`Mohon Menunggu Beberapa Detik Kawan ツ`')
    await asyncio.sleep(10)
    await event.delete()
    ...
    if BOTLOG:
        await event.client.send_message(
            BOTLOG_CHATID, "#BOT \n"
            "**✵Lord-RYU Telah Di Perbarui kawan ツ**")
...
async def upstream(event):
    ...
    if conf is None and force_update is False:
        changelog_str = f'**✣ Pembaruan Untuk Lord-ryu [{ac_br}]:\n\n✣ Pembaruan:**\n`{changelog}`'
    ...
    if force_update:
        ...
        await event.edit('`✲ Proses Update Lord-RYU, Loading....20%`')
        await event.edit('`✲ Proses Update Lord-RYU, Loading....35%`')
        await event.edit('`✲ Proses Update Lord-RYU, Loading....77%`')
        await event.edit('`✲ Proses Update Lord-RYU, Updating...90%`')
        await event.edit('`✲ Proses Update Lord-RYU, Mohon Menunggu Kawan....100%`')
    if conf == "now":
...
CMD_HELP.update({
    'update':
    ".update"
    "\nUsage: Untuk Melihat Pembaruan Terbaru Lord-ryu."
    "\n\n.update now"
    "\nUsage: Memperbarui Lord-ryu."
    "\n\n.update deploy"
    "\nUsage: Memperbarui Lord-ryu Dengan Cara Deploy Ulang."
})
