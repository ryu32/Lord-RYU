...
@register(outgoing=True, pattern=r'^.goku(:? |$)([1-8])?')
async def _(fry):
    await fry.edit("`Lord Mengaktifkan Kekuatan goku kamehameha...👾`")
    level = fry.pattern_match.group(2)
    if fry.fwd_from:
        return
    if not fry.reply_to_msg_id:
        await fry.edit("`Mohon Balas Di Sticker kawan`")
        return
